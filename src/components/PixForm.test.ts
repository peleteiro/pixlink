...
describe("construirUrl", () => {
  describe("erros", () => {
    ...
    it("retorna erro 'valor-invalido' quando valor e vazio", () => {
      expect(construirUrl("11912345678", "")).toEqual({
        erro: "valor-invalido",
      });
    });

    it("retorna erro 'valor-invalido' quando valor nao e numero", () => {
      expect(construirUrl("11912345678", "abc")).toEqual({
        erro: "valor-invalido",
      });
    });

    it("retorna erro 'valor-invalido' quando valor e zero", () => {
      expect(construirUrl("11912345678", "0")).toEqual({
        erro: "valor-invalido",
      });
    });
  });
  ...
  describe("formato do valor na URL", () => {
    it("usa virgula com dois digitos de centavos", () => {
      expect(construirUrl("11912345678", "50,00")).toEqual({
        url: "/11912345678/50,00",
      });
    });

    it("aceita valor inteiro sem decimais", () => {
      expect(construirUrl("11912345678", "50")).toEqual({
        url: "/11912345678/50,00",
      });
    });

    it("aceita valor abaixo de 1 real", () => {
      expect(construirUrl("11912345678", "0,50")).toEqual({
        url: "/11912345678/0,50",
      });
    });

    it("arredonda fracao de centavo para o inteiro mais proximo", () => {
      expect(construirUrl("11912345678", "0,015")).toEqual({
        url: "/11912345678/0,02",
      });
    });

    it("ignora prefixo R$", () => {
      expect(construirUrl("11912345678", "R$50,00")).toEqual({
        url: "/11912345678/50,00",
      });
    });
    ...
    it("ignora ponto como separador de milhar", () => {
      expect(construirUrl("11912345678", "50.000,00")).toEqual({
        url: "/11912345678/50000,00",
      });
    });
    ...
    it("trata ponto final seguido de 1 digito como virgula", () => {
      expect(construirUrl("11912345678", "50.5")).toEqual({
        url: "/11912345678/50,50",
      });
    });
  ...
  describe("encoding da chave", () => {
    it("url-encoda e-mail com @ e +", () => {
      expect(construirUrl("joana+c6@example.com", "50")).toEqual({
        url: "/joana%2Bc6%40example.com/50,00",
      });
    });
    ...
    it("url-encoda + do DDI no telefone", () => {
      expect(construirUrl("+5511912345678", "50")).toEqual({
        url: "/%2B5511912345678/50,00",
      });
    });
  });
  ...
  describe("descricao", () => {
    ...
    it("adiciona descricao como query param ?d=", () => {
      expect(construirUrl("11912345678", "50", "Almoco")).toEqual({
        url: "/11912345678/50,00?d=Almoco",
      });
    });
    ...
    it("url-encoda espacos e caracteres especiais na descricao", () => {
      expect(construirUrl("11912345678", "50", "Almoco R$50")).toEqual({
        url: "/11912345678/50,00?d=Almoco%20R%2450",
      });
    });
  });
});
