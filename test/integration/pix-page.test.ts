...
describe("integracao: pagina /[chave]/[valor]", () => {
  ...
  it("renderiza pagina valida com 200 e QR Code", async () => {
    const res = await render(
      ...
      "https://pix.peleteiro.net/11912345678/50,00",
    );
  ...
  it("meta og:image aponta para a rota .png canonica", async () => {
    ...
    const html = await res.text();
    expect(html).toContain(
      'property="og:image" content="https://pix.peleteiro.net/%2B5511912345678/50,00.png"',
    );
  });

  it("retorna 400 com pagina de erro quando valor e invalido", async () => {
    const res = await render(
      { chave: "11912345678", valor: "abc" },
      "https://pix.peleteiro.net/11912345678/abc",
    );
  ...
  it("renderiza com chave CPF e label correto", async () => {
    const res = await render(
      { chave: "111.444.777-35", valor: "15" },
      "https://pix.peleteiro.net/111.444.777-35/15",
    );
    expect(res.status).toBe(200);
    const html = await res.text();
  ...
  it("exibe a descricao na pagina quando presente", async () => {
    ...
    expect(html).toContain('content="Telefone: +55 (11) 91234-5678 — Almoco"');
  });
  ...
  it("trunca descricao longa no payload PIX (72 chars max)", async () => {
    ...
    const res = await render(
      { chave: "11912345678", valor: "50,00" },
      `https://pix.peleteiro.net/11912345678/50,00?d=${longa}`,
    );
    const html = await res.text();
  ...
  it("rejeita valor acima do limite seguro", async () => {
    const res = await render(
      { chave: "11912345678", valor: "99999999999999999" },
      "https://pix.peleteiro.net/11912345678/99999999999999999",
    );
    expect(res.status).toBe(400);
  });
});
